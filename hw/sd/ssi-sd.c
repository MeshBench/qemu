...
struct ssi_sd_state {
    ...
    uint8_t response[5];
    /*
     * The last R1 the card actually reported, so an R3 or R7 can carry a
     * true one. Not migrated: it is regenerated by the next command, and a
     * version bump for one transient byte is not worth what it costs every
     * other user of this device.
     */
    uint8_t r1;
    uint16_t crc16;
    int32_t read_bytes;
...
static uint32_t ssi_sd_transfer(SSIPeripheral *dev, uint32_t val)
{
    ...
    case SSI_SD_CMDARG:
        if (s->arglen == 4) {
            ...
            if (s->arglen <= 0) {
                ...
                s->arglen = 5;
                /*
                 * The R1 the card last reported, not a constant 1. Hardcoded,
                 * the leading byte of every R3 and R7 said "in idle state"
                 * for ever - so a host that finished ACMD41 and then read the
                 * OCR with CMD58 was told the card was still idle, and gave up
                 * on a card that had just initialised perfectly well. CMD8 is
                 * unaffected: it is asked while the card really is idle.
                 */
                s->response[0] = s->r1;
    ...
    case SSI_SD_RESPONSE:
        if (s->response_pos < s->arglen) {
            uint8_t rb = s->response[s->response_pos++];
            DPRINTF("Response 0x%02x\n", rb);
            /*
             * Finish here rather than on the following clock, where there is
             * nothing else to wait for. A card does not need to be clocked to
             * notice it has said everything it had to say, and a host may put
             * the next command out immediately behind the response - the
             * specification's Ncc is a minimum for the host, not something the
             * card enforces.
             *
             * Waiting for one more byte meant that command was eaten: an
             * ESP32-S3 firmware sending CMD8 straight after CMD0's R1 had its
             * 0x48 swallowed here, its 0x00 taken as the command, and its
             * argument read a byte late - which came back as an unsupported
             * CMD0 and stopped the card's initialisation dead, several layers
             * away from anything mentioning SPI timing.
             *
             * Only the no-data case short-circuits. Where a read is pending
             * the old path still runs, so the data token keeps its timing.
             */
            if (s->response_pos == s->arglen && !s->stopping &&
                !sdbus_data_ready(&s->sdbus)) {
...
static void ssi_sd_reset(DeviceState *dev)
{
    ...
    s->response_pos = 0;
    s->stopping = 0;
    /* Idle out of reset, which is what a card reports until it is told
     * otherwise. */
    s->r1 = SSI_SDR_IDLE >> 8;
}
